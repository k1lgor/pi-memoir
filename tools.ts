...
const memoStoreParams = Type.Object({
  content: Type.String({
    description: "The memory content to store (what you want to remember)",
  }),
  summary: Type.Optional(
    Type.String({
      description:
        "A short summary/headline (default: first 80 chars of content)",
    }),
  ),
  tags: Type.Optional(
    Type.String({
      description:
        "Comma-separated tags for categorization, e.g. 'architecture,decision,config' or 'project:overview'",
    }),
  ),
});
...
const memoSearchParams = Type.Object({
  query: Type.String({
    description:
      "Search keywords to find relevant memories. Use project-specific terms like file names, architecture concepts, or tech stack names.",
  }),
  tags: Type.Optional(
    Type.String({
      description:
        "Optional: comma-separated tags to filter by (e.g. 'project:structure,project:config'). Only memories with ALL specified tags are returned.",
    }),
  ),
  limit: Type.Optional(
    Type.Number({
...
const memoSearchTool: ToolDefinition<typeof memoSearchParams> = {
  ...
  promptSnippet:
    "REPLACES bash/ls/read for project exploration — saves ~20x tokens",
  promptGuidelines: [
        ...
        content: [
          {
            type: "text" as const,
            text:
              "No matching memories found. The project may not have been harvested yet. Try:\n" +
...
const memoHarvestTool: ToolDefinition<typeof memoHarvestParams> = {
  ...
  promptSnippet:
    "Scan and memorize the entire project (replaces reading all files)",
  promptGuidelines: [
