...
const KEY_FILES: KeyFile[] = [
  ...
  },
  {
    ...
    tag: "project:config",
    label: "TypeScript app config",
  },
  {
    priority: 3,
    filename: "tsconfig.node.json",
    tag: "project:config",
    label: "TypeScript node config",
  },
  {
    priority: 3,
...
const SOURCE_EXTS = new Set([
  ...
  ".jade",
  ".haml",
  ".slim", // HTML preprocessors
  ".ejs",
  ".hbs",
  ".handlebars",
  ".mustache", // JS template engines
  ".njk",
  ".nunjucks",
  ".liquid", // Jinja/Nunjucks/Liquid
  ".erb",
  ".slim", // Ruby templates
  ".ctp", // CakePHP
  ...
  ".rst",
  ".adoc",
  ".asciidoc", // reStructuredText, AsciiDoc
  ".tex",
  ".latex",
  ".ltx",
  ".bib", // LaTeX
  ...
  ".npmrc",
  ".babelrc",
  ".eslintrc", // will match CONFIG_PATTERNS too
  ".prettierrc",
  ".dhall",
...
// Improvement 6: Chunked Large Files
...
export async function harvestProject(projectDir: string): Promise<number> {
  // Improvement 4: Deduplicated Knowledge - track seen paths to avoid duplicates
  ...
  count += await harvestAllSourceFiles(projectDir, seenPaths);
  ...
  for (const kf of KEY_FILES) {
    ...
    try {
      ...
      let tags = [
        "project",
        kf.tag,
        kf.filename.toLowerCase().replace(/\./g, ":"),
      ];
...
function walkDirectory(
  ...
  const files = entries
    .filter(
      (e) => e.isFile() && !SKIP_FILES.has(e.name) && !e.name.startsWith("."),
    )
    .sort((a, b) => a.name.localeCompare(b.name));
  ...
  const scanDir = async (dir: string, depth: number): Promise<void> => {
    ...
    for (const entry of entries) {
      ...
      if (
        ...
        KEY_FILES.some((kf) => kf.filename === entry)
      )
        continue;
      ...
      try {
        ...
        await store.store({
          ...
          tags: [
            "project",
            "project:config",
            `file:${relPath.replace(/[/\\]/g, ":").replace(/\./g, ":")}`,
          ],
          source: "auto",
        });
        count++;
...
async function harvestAllSourceFiles(
  projectDir: string,
  ...
  const walk = async (dir: string): Promise<void> => {
    ...
    for (const e of entries) {
      ...
      // Improvement 6: Chunked Large Files - split large files into chunks
      if (stat.size > MAX_FILE_SIZE) {
        // Split into chunks and store each chunk as separate memory
        try {
          const fullContent = fs.readFileSync(fullPath, "utf-8");
          const chunks = splitIntoChunks(fullContent, CHUNK_SIZE);
          const relPath = path.relative(projectDir, fullPath);

          for (let i = 0; i < chunks.length && count < MAX_FILE_MEMORIES; i++) {
            const chunkSummary = `${relPath} [chunk ${i + 1}/${chunks.length}] (${chunks[i].length} chars)`;
            const chunkTags = generateSemanticTags(chunks[i], relPath);

            await store.store({
              content: `File: ${relPath} (chunk ${i + 1}/${chunks.length})\n\n${chunks[i]}`,
      ...
      try {
        ...
        if (ext === ".json") {
          ...
          try {
            ...
            summary = `${relPath} (${lines.length} lines)`;
          }
        } else if (ext === ".md") {
          ...
          const firstSig = lines.find((l) =>
            /^(import|export|function|class|const|let|var|def|pub|fn)\s/.test(
              l,
            ),
          );
        ...
        // Improvement 4: Deduplicated Knowledge - skip if we've seen this path
        if (seenPaths.has(relPath)) {
          continue;
        }
        seenPaths.add(relPath);

        // Improvement 7: Semantic Tags Auto-Generation
        const semanticTags = generateSemanticTags(content, relPath);
        const allTags = [
          "project",
          "project:file",
          `file:${relPath.replace(/[/\\]/g, ":").replace(/\./g, ":")}`,
...
function detectLanguage(pkg: Record<string, unknown>): string {
  ...
  if (/\breact\b|\bnext\b|\bvue\b|\banguler\b|\bsvelte\b/i.test(allDeps))
    return "typescript";
  if (/\bexpress\b|\bkoa\b|\bfastify\b|\bnest\b/i.test(allDeps))
    return "typescript";
  if (/\bjest\b|\bvitest\b|\beslint\b|\bprettier\b/i.test(allDeps))
    return "typescript";
  if (/\bpython\b|\bdjango\b|\bflask\b|\bfastapi\b/i.test(allDeps))
    return "python";
...
/**
 * Improvement 6: Split large content into chunks for indexing
 */
function splitIntoChunks(content: string, chunkSize: number): string[] {
  ...
  if (
    chunks.length === 0 ||
    chunks[chunks.length - 1].length > chunkSize * 1.5
  ) {
    return content.match(new RegExp(`.{1,${chunkSize}}`, "g")) || [content];
  }
...
/**
 * Improvement 7: Auto-generate semantic tags based on content patterns
 */
function generateSemanticTags(content: string, filePath: string): string[] {
  const tags: string[] = [];
  const lowerContent = content.toLowerCase();

  // API/Network patterns
  if (
    /\b(fetch|axios|XMLHttpRequest|Request|Response)\b/.test(lowerContent) ||
    /\b(https?:\/\/[^\s]+|api(?=\/|\.|$))/.test(content)
  ) {
    tags.push("api:http");
  }

  // WebSocket patterns
  if (/\b(ws:|wss:|websocket|WebSocket)\b/.test(lowerContent)) {
    tags.push("api:websocket");
  }

  // GraphQL patterns
  if (/\b(query|mutation|subscription|graphql|__schema)\b/.test(lowerContent)) {
    tags.push("api:graphql");
  }

  // Database patterns
  if (
    /\b(sqlite|mysql|postgres|postgresql|mongodb|redis|sql|SELECT|INSERT|UPDATE|DELETE|CREATE TABLE)\b/i.test(
      lowerContent,
    )
  ) {
    tags.push("db:sql");
  }
  if (/\b(prisma|typeorm|sequelize|mongoose|knex)\b/i.test(lowerContent)) {
    tags.push("db:orm");
  }

  // Config/Env patterns
  if (
    /\b(process\.env|import\.meta\.env|DOTENV|dotenv|getenv|os\.environ)\b/.test(
      lowerContent,
    ) ||
    /\b(API_KEY|SECRET|PASSWORD|TOKEN|KEY)=/.test(content)
  ) {
    tags.push("config:env");
  }

  // Authentication patterns
  if (
    /\b(jwt|oauth|passport|session|cookie|login|auth|authenticate|authorize)\b/i.test(
      lowerContent,
    )
  ) {
    tags.push("auth");
  ...
  // Testing patterns
  if (
    /\b(describe|it|test|expect|jest|vitest|mocha|cypress|playwright)\b/.test(
      lowerContent,
    )
  ) {
    tags.push("test");
  ...
  if (
    /\b(async|await|Promise|then|catch|finally|setTimeout|setInterval)\b/.test(
      lowerContent,
    )
  ) {
    tags.push("async");
  ...
  // UI/Component patterns (common frameworks)
  if (
    /\b(React|Component|useState|useEffect|useRef|render|return\s*\(|View|Text|Button|div|span)\b/.test(
      content,
    )
  ) {
  ...
  if (
    /\b(Vue|computed|watch|ref|reactive|template|script|style)\b/.test(content)
  ) {
    tags.push("ui:component");
  }
  if (
    /\b(Svelte|export\s+let|onMount|onDestroy|script|style)\b/.test(content)
  ) {
    tags.push("ui:component");
  }
  ...
  if (
    /\b(redux|zustand|store|createStore|useStore|useReducer|context|Provider)\b/i.test(
      lowerContent,
    )
  ) {
    tags.push("state");
  ...
  // Type definitions
  if (
    /\b(interface|type\s+\w+|enum|typedef|struct|class\s+\w+\s*\{)\b/.test(
      content,
    )
  ) {
    tags.push("types");
  ...
  if (ext === "ts" || ext === "tsx") tags.push("lang:typescript");
  else if (ext === "js" || ext === "jsx") tags.push("lang:javascript");
  else if (ext === "py") tags.push("lang:python");
  else if (ext === "rs") tags.push("lang:rust");
  else if (ext === "go") tags.push("lang:go");
  else if (ext === "md") tags.push("lang:markdown");
  else if (ext === "json") tags.push("lang:json");
  else if (ext === "sql") tags.push("lang:sql");
